...
class HabitMapperTest {
    ...
    fun `habit entity toDomainModel maps all fields`() {
        val startDate = LocalDate(2025, 1, 20)
        val endDate = LocalDate(2025, 2, 1)
        val entity =
            HabitEntity(
                id = "habit-1",
                title = "Read",
        ...
        assertEquals(LocalTime(9, 30), model.reminderTime)
    ...
    fun `habit toEntity maps all fields and legacy goalCount`() {
        val habit =
            Habit(
                ...
                endDate = LocalDate(2025, 4, 1),
                reminderEnabled = true,
                reminderTime = LocalTime(6, 15),
                createdAt = 30L,
                updatedAt = 40L,
                isArchived = false,
                sortOrder = 2,
        ...
        assertEquals(toMillis(LocalDate(2025, 3, 1)), entity.startDate)
        assertEquals(toMillis(LocalDate(2025, 4, 1)), entity.endDate)
    ...
    fun `habit log toEntity maps fields and legacy counts`() {
        ...
        assertEquals(toMillis(LocalDate(2025, 5, 2)), entity.date)
    ...
    fun `habit entity handles null optional fields`() {
        val entity =
            HabitEntity(
                ...
                startDate = toMillis(LocalDate(2025, 1, 1)),
            )
    ...
    fun `habit log entity parses completed checklist items json`() {
        val entity =
            HabitLogEntity(
                id = "log-3",
                habitId = "habit-3",
                date = toMillis(LocalDate(2025, 2, 2)),
