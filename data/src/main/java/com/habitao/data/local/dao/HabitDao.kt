...
interface HabitDao {
    ...
    suspend fun setHabitArchived(habitId: String, isArchived: Boolean)
}
...
interface HabitLogDao {
    ...
    suspend fun getLogsBetweenDates(startDate: Long, endDate: Long): List<HabitLogEntity>
    
    // ============== SPECIFIC DATE ==============
    
    @Query("SELECT * FROM habit_logs WHERE habitId = :habitId AND date = :date")
    suspend fun getLogForHabitAndDate(habitId: String, date: Long): HabitLogEntity?
    ...
    @Query("""
        ...
        LIMIT :limit
    """)
    suspend fun getCompletedLogsForStreak(habitId: String, limit: Int): List<HabitLogEntity>
    ...
    @Query("""
        ...
        AND isCompleted = 1
    """)
    suspend fun getCompletionsBetweenDates(habitId: String, startDate: Long, endDate: Long): Int
}
